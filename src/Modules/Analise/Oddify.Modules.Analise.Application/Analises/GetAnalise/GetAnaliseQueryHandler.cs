...
internal sealed class GetAnaliseQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<GetAnaliseQuery, AnaliseResponse>
{
    public async Task<Result<AnaliseResponse>> Handle(GetAnaliseQuery request, CancellationToken cancellationToken)
    {
        ...
        AnaliseResponse? result = await connection.QuerySingleOrDefaultAsync<AnaliseResponse>(sql, request);

        if (result is null)
        {
            return Result.Failure<AnaliseResponse>(AnaliseDePartidaErrors.NotFound(request.AnaliseId));
        }
