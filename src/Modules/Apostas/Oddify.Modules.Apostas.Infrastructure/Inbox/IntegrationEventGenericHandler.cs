...
// O único handler que efetivamente assina o InMemoryEventBus (ver ApostasModule.Initialize) —
// genérico sobre qualquer IIntegrationEvent que este módulo consome. Não processa nada: só grava a
// mensagem em apostas.inbox_messages e retorna — dá durabilidade do lado de quem recebe antes de
    ...
    where TIntegrationEvent : IIntegrationEvent
{
    ...
    public override async Task Handle(TIntegrationEvent integrationEvent, CancellationToken cancellationToken = default)
    {
        ...
        // is of type text".
        const string sql =
            "INSERT INTO apostas.inbox_messages(id, type, content, occurred_on_utc) VALUES (@Id, @Type, @Content::jsonb, @OccurredOnUtc)";
