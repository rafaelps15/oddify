namespace Oddify.Modules.Fixtures.Presentation;

internal static class Tags
{
    internal const string Ligas = "Ligas";
    internal const string Equipes = "Equipes";
    internal const string Jogadores = "Jogadores";
    internal const string Partidas = "Partidas";
    internal const string Cotacoes = "Cotacoes";
    internal const string EstatisticasDeEquipe = "EstatisticasDeEquipe";
    internal const string EstatisticasDeJogador = "EstatisticasDeJogador";
}
